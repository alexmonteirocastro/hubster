...
- Remote negation detection uses a fixed phrase list and a small negation-window heuristic (not general NLP). Phrasings outside that closed set may be missed entirely (falls back to no filter), and idiomatic double negatives ("no problem with remote", "not opposed to remote") may be misread as `remote=False` by the negation window unless listed in the neutral-phrase table — both directions degrade to unfiltered search or an explicit `remote` param, not a wrong confident answer, when handled via `REMOTE_NEUTRAL_PHRASES`.
